...
object Bacon 
{
	...
	def main(args: Array[String]) 
	{
		...
		movie2actor.persist(if (compressRDDs) MEMORY_ONLY_SER else MEMORY_ONLY)
		//(movie, (actor, actor))
		...
		// =====================================================================================================================================
		// GraphX library code is here below.
		// =====================================================================================================================================

		// Vertice (id, actor_name)
		val vertices_Rdd: RDD[(VertexId, String)] = actor2ListOfMovies1.map{case((a, ml), i) => (i, a)}
		// Edge (actor_id1, actor_id2, "movie")
		val edges_Rdd: RDD[Edge[(VertexId, VertexId)]] = actor2actor.map(x => Edge(x._1, x._2))
		// Graph (vertives, edges)
		val graph = Graph(vertices_Rdd, edges_Rdd)

		// Getting shortest distance for each vertices which leads to kevin
		val shortestPaths = ShortestPaths.run(graph, Seq(KevinBaconID))

		// (actor, distance)
		var actor2distance = shortestPaths.vertices.map(x => (x._1, x._2.get(KevinBaconID))).filter(x => x._2 != None).map(x => (x._1, x._2.get))
		actor2distance.setName("rdd_actor2distance")
		actor2distance.persist(if (compressRDDs) MEMORY_ONLY_SER else MEMORY_ONLY)

		// =====================================================================================================================================
		// GraphX library code is above.
		// =====================================================================================================================================

		val countListActors = ArrayBuffer[Long]()
		val countListActresses = ArrayBuffer[Long]()
		var dist = 0
		// prepare collections for all actors whith distance between 0 and 6
		for (dist <- 0 until Distance)
		{
			countListActors.append(actor2distance.filter(x => x._1 > 0 && x._2 == dist+1).count)
			countListActresses.append(actor2distance.filter(x => x._1 < 0 && x._2 == dist+1).count)
		}
		// prepare the output data, for example the percetage of the actress
		...
		val x6 = actorID2actor.join(actor2distance.filter(x => x._2 == 6)).map{x => 
